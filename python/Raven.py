...
class Raven:
    ...
    def _readXMLfragment(self) -> str:
        ...
        while (start_tag[-1] != 62):
            start_tag += self._raven.read()
        start_tag = start_tag.decode('ascii').strip().encode('ascii')
        result = start_tag
        end_tag = start_tag[0:1] + '/'.encode('ascii') + start_tag[1:] 
        end_tag_len = len(end_tag)
        while (result[-end_tag_len:] != end_tag):
            result += self._raven.read()
        return ET.fromstring(result.decode('ascii'))
